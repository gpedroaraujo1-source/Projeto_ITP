...
TEST_CASE("Testa a adição de cores uma paleta vazia") {
    ...
    CHECK(paleta.getNumeroCores() == 1);
    CHECK(paleta.getCorIndice(0).r == 255);
    CHECK(paleta.getCorIndice(0).g == 0);
    CHECK(paleta.getCorIndice(0).b == 0);
}
...
TEST_CASE("Testa a obtenção de uma cor com índice inválido") {
    ...
    Cor corInvalida = paleta.getCorIndice(5); // índice inválido (maior que o tamanho)
...
TEST_CASE("Testa a criação de uma paleta a partir de um arquivo válido") {
    ...
    CHECK(paleta.getNumeroCores() == 30);
    CHECK(paleta.getCorIndice(0).r == 113);
    CHECK(paleta.getCorIndice(0).g == 171);
    CHECK(paleta.getCorIndice(0).b == 216);

    CHECK(paleta.getCorIndice(1).r == 121);
    CHECK(paleta.getCorIndice(1).g == 178);
    CHECK(paleta.getCorIndice(1).b == 222);
    
    //... outras cores

    CHECK(paleta.getCorIndice(29).r == 255);
    CHECK(paleta.getCorIndice(29).g == 255);
    CHECK(paleta.getCorIndice(29).b == 255);
}
